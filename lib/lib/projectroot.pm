...
There is C<AProject>, which is the actual code I'm working on. There is also probably C<BProject>, e.g. another microservice for the same customer. C<A-Project> has it's own code in C<lib> and it's CPAN dependencies in C<local> (managed via C<Carton> and used via C<local::lib>). There are a bunch of scripts / "binaries" in C<bin>, in a lot of different directories of varying depth.
